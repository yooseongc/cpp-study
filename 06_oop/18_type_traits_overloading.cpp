...
namespace dmc
{
    ...
    constexpr std::true_type is_a_matrix(const dense_matrix<Value>&) { return {}; }

    template <typename Value>
    constexpr std::true_type is_a_matrix(const sparse_matrix<Value>&) { return {}; }
    ...
    template <typename Matrix, typename Functor>
    constexpr std::true_type is_a_matrix(const map_view<Matrix, Functor>&) { return {}; }
    ...
    void check_matrix()
    {
        std::cout << typeid(T).name() << " is " << (is_matrix<T>{} ? "" : "not ") << "a matrix." << std::endl;
