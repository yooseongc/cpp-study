...
namespace dmc
{
    ...
    constexpr bool commutative_monoid_map 
        = decltype(commutative_monoid_map_impl(std::declval<Op>(), std::declval<Value>()))::value;
    ...
    {
        std::cout << "Ordinary accumulate with " << typeid(first).name() << std::endl;
    ...
    Value accumulate(Iter first, Iter last, Value init, Op op)
        requires Accumulatable<Iter, Value, Op> && CommutativeMonoid<Op, Value>
    {
        std::cout << "Unrolled implementation with " << typeid(first).name() << std::endl;
