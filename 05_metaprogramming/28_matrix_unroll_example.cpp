...
namespace dmc
{
    ...
    public:
        ...
        matrix(int nrows, int ncols) : nrows(nrows), ncols(ncols), data(new value_type[nrows * ncols]) {}
        matrix(const matrix& that) : nrows(that.nrows), ncols(that.ncols), data(new value_type[nrows * ncols])
        {
    ...
    struct mult_block
    {
        ...
        static void eval(Tmp& tmp, const Matrix& A, const Matrix& B, unsigned i, unsigned j, unsigned k)
        {
        ...
        template <typename Tmp, typename Matrix>
        static void update(const Tmp& tmp, Matrix& C, unsigned i, unsigned k)
        {
    ...
    void bench(const Matrix& A, const Matrix& B, Matrix& C, const unsigned rep)
    {
        ...
        std::cout << "Time mult is " << 1000000.0 * t << " ms. This are "
                  << s * s * (2 * s - 1) / t / 1000000.0 << "MFlops." << std::endl;
    ...
    void bench_unroll(const Matrix &A, const Matrix &B, Matrix &C, const unsigned rep)
    {
        ...
        std::cout << "Time mult<" << Size0 << ", " << Size1 << "> is " << 1000000.0 * t << " ms. This are "
                  << s * s * (2 * s - 1) / t / 1000000.0 << "MFlops." << std::endl;
