...
namespace dmc
{
    ...
    template <typename Op, typename Value>
    concept SemiGroup = Magma<Op, Value> && semi_group_map<Op, Value>;

    template <typename Op, typename Value>
    concept Monoid = SemiGroup<Op, Value> && 
    ...
    template <typename Op, typename Value>
    concept Commutative = Magma<Op, Value> && commutative_map<Op, Value>;

    template <typename Op, typename Value>
    concept CommutativeMonoid = Monoid<Op, Value> && Commutative<Op, Value>;
    ...
    {
        std::cout << "CommutativeMonoid." << std::endl;
    }
...
{
    std::cout << "(" << typeid(Op).name() << ", " << typeid(Value).name() << ") model SemiGroup." << std::endl;
}
