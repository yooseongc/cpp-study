...
template <typename T>
vector_sum<T> inline operator+(const vector<T>& x, const vector<T>& y) { return {x, y}; }
...
template <typename T>
std::ostream &operator<<(std::ostream &os, const vector_sum3<T>& v)
{
    os << '[';
    if (size(v) > 0)
        os << v[0];
...
{
    x.check_index(size(y)); x.check_index(size(z)); x.check_index(size(sum));
    for(int i = 0; i < size(x); ++i) sum[i] = x[i] + y[i] + z[i];
}
